...
class ShopfloorController extends Controller
{
    public function index()
    {
        if (Auth::user()->hasPermissionTo('Shopfloor View')) {
            return view('mes.dashboard.shopfloor');
        }
    ...
    {
        $machines = Machine::select('id', 'name', 'code')->whereIn('name', ['B3', 'B2', 'B1', 'M1', 'M2', 'M3', 'M4', 'M5', 'M6', 'M7', 'M8', 'M9', 'M10', 'M11', 'M12', 'M13', 'M14'])->get();
        $machines_array = [];
        foreach ($machines as $key => $machine) {
            ...
            $machine_downtime = MachineDownime::where('mc_no', $machine->code)->orderBy('id', 'DESC')->first();
            ...
            if ($machine_downtime) {
                if ($machine_downtime->end_time == null) {
                    $color = 'danger';
                }
            }
            $machines_array[] = [
