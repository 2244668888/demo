...
@section('content')
                        ...
                        <table class="table table-bordered m-0" id="p_table">
                            ...
                            <tbody>
                                ...
                                @php
                                    ...
                                    $parent_product[$loop->iteration]['di_qty'] = $dppp->di_qty;
                                @endphp
                                    <tr>
                                        <td>
                                            {{ $loop->iteration }}
    ...
    <script>
        ...
        function processBomData(bomData, parentProductId, parentQty, schedulingTable,level = 0) {
            bomData.forEach((bom, index) => {
                ...
                bom.processes.forEach((process, processIndex) => {
                    ...
                    var subPartNames = '';
                    if (json_decodedsubparts) {
                        subPartNames = json_decodedsubparts.map(subPartId => {
                            const product = all_products.find(product => product.id ==
                                subPartId);
                    ...
                    const json_decodedchildPart = JSON.parse(process.raw_part_ids);
                        var childPartNames = '';
                        if (json_decodedchildPart) {
                            childPartNames = json_decodedchildPart.map(childPartId => {
                                const product = all_products.find(product => product.id ==
                                    childPartId);
                                return product ? product.part_name : '';
                            }).filter(name => name !== '').join(', ');
        ...
        function addRow(button) {
            ...
            $('#planningTable').DataTable().row.add(rowData).draw(false);
