...
@section('content')
    ...
    <form method="post" action="{{ route('material_requisition.received', $material_requisition->id) }}"
        ...
        <div class="card">
            <div class="card-body">
                ...
                <div class="row">
                    <div class="col-lg-3 col-sm-4 col-12">
                        <div class="mb-3">
                            ...
                            <input type="text" disabled id="issue_by" value="{{ $user->user_name }}"
                                class="form-control">
                        </div>
                    </div>
                    <div class="col-lg-3 col-sm-4 col-12">
                        <div class="mb-3">
                            ...
                            <input type="date" disabled name="issue_date" id="issue_date"
                                value="{{ $material_requisition->issue_date }}" class="form-control">
                        </div>
                    </div>
                    <div class="col-lg-3 col-sm-4 col-12">
                        <div class="mb-3">
                            <label for="time" class="form-label">Issue Time</label>
                            <input type="time" disabled name="issue_time"
                                value="{{ $material_requisition->issue_time }}" id="time" class="form-control" />
                        </div>
                    </div>
                    <div class="col-lg-3 col-sm-4 col-12">
                        <div class="mb-3">
                            <label for="issue_remarks" class="form-label">Remarks</label>
                            <input type="text" disabled name="issue_remarks" id="issue_remarks"
                                value="{{ $material_requisition->issue_remarks }}" class="form-control">
                        </div>
                    </div>
                </div>
                <br>
                ...
                <div class="row">
                    <div class="col-lg-3 col-sm-4 col-12">
                        <div class="mb-3">
                            ...
                            <input type="text" disabled id="rcv_by" value="{{ Auth::user()->user_name }}"
                                class="form-control">
                            <input type="hidden" name="rcv_by" id="rcv_by" value="{{ Auth::user()->id }}"
                                class="form-control">
                        </div>
                    </div>
                    <div class="col-lg-3 col-sm-4 col-12">
                        <div class="mb-3">
                            ...
                            <input type="date" readonly name="rcv_date" id="rcv_date" value="{{ date('Y-m-d') }}"
                                class="form-control">
                        </div>
                    </div>
                    <div class="col-lg-3 col-sm-4 col-12">
                    ...
                    <div class="col-lg-3 col-sm-4 col-12">
                        <div class="mb-3">
                            ...
                            <input type="text" name="rcv_remarks" id="rcv_remarks" class="form-control">
                        </div>
                    </div>
                </div>
                <br>
                ...
                <div class="row">
                    <div class="col-12">
                        <div class="table-responsive" id="popUpMain">
        ...
        {{-- REMARKS MODAL 2 --}}
        <div class="modal fade" id="remarksModal2" tabindex="-1" aria-labelledby="remarksModalLabel"
            aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
        ...
        {{-- REMARKS MODAL 3 --}}
        <div class="modal fade" id="remarksModal3" tabindex="-1" aria-labelledby="remarksModalLabel"
            aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
    ...
    <script>
                ...
                if (materialRequisitionDetails.find(detail => detail.product_id == $(this).val())) {
                    ...
                    if (matchedDetail) {
                        ...
                        if (issue_remarks == '' || issue_remarks == null) {
                            issue_remarks_button =
                                `<button type="button" class="btn btn-sm add-remarks2 btn-danger" disabled>No Remarks</button>`;
                        } else {
                            issue_remarks_button =
                                `<button type="button" class="btn btn-sm add-remarks2 btn-success">View</button>`;
                        }
                        if (rcv_remarks == '' || rcv_remarks == null) {
                            rcv_remarks_button =
                                `<button type="button" class="btn btn-sm add-remarks3 btn-danger">Add</button>`;
                        } else {
                            rcv_remarks_button =
                                `<button type="button" class="btn btn-sm add-remarks3 btn-success">Edit</button>`;
                        }
                    ...
                    if ($('#productTable tbody input[value="' + productId + '"]').length == 0) {
                        if (material_requisition_product_details.length === 0) {
                            ack_reg_btn =
                                `<a href="" class="btn btn-sm btn-primary ack" data-mrf-detail-id="${mrf_detail_id}" title="Acknowledge"><i class="bi bi-check-circle"></i></a> <a href="" data-mrf-detail-id="${mrf_detail_id}" class="btn btn-sm btn-danger reject" title="Reject"><i class="bi bi-x-square" t></i></a>`;
                        } else {
                            $.each(material_requisition_product_details, function(key, detail) {
                                if (detail.product_id == productId) {
                                    if (detail.area == 2) {
                                        ack_reg_btn =
                                            `<button type="button" class="btn btn-sm add-remarks2 btn-danger" disabled>Rejected</button>`;
                                    }
                                    if (detail.area == 1) {
                                        ack_reg_btn =
                                            `<button type="button" class="btn btn-sm add-remarks2 btn-success" disabled>Received</button>`;
                                    }
                                    return false;
                                } else {
                                    ack_reg_btn =
                        ...
                        let rowNode = productTable.row.add([
                            ...
                            `<input name="product[${productTable.rows().count() + 1}][rcv_qty]" class="rcv_qty form-control" type="number" step="1" value="${rcv_qty}">`,
                            ...
                            `<div class="d-flex">${ack_reg_btn}</div>`
                        ]).draw(false).node();

                        setTimeout(function() {
                            $(rowNode).find('.rcv_qty').attr('step', '1');
                        }, 1000); // Delay of 3 seconds

                        // Attach event listener to dynamically update the href
                        $('#productTable tbody').on('click', '.ack', function(e) {
                            e.preventDefault();

                            // Get mrf_detail_id from data attribute
                            let mrf_detail_id_ack = $(this).data('mrf-detail-id');

                            // Get the corresponding qty value from the same row
                            let rcv_qty_text = $(this).closest('tr').find('.rcv_qty')
                        .val(); // Assuming .rcv_qty is the input for qty
                            let issue_qty_text = $(this).closest('tr').find('.issue_qty')
                        .val(); // Assuming .rcv_qty is the input for qty
                            let rcv_qty = Number(rcv_qty_text);
                            let issue_qty = Number(issue_qty_text);
                            if (rcv_qty > issue_qty) {
                                $('#popUpMain').prepend(`
                                    <div class="alert border-danger alert-dismissible fade show text-danger" role="alert">
                                    <b>Error !</b> Receive quantity can't be greater then issued quantity !.
                                    <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                                    </div>
                                `);
                                $(this).closest('tr').find('.rcv_qty').val(issue_qty);
                            }
                            if (rcv_qty <= 0) {
                                $('#popUpMain').prepend(`
                                    <div class="alert border-danger alert-dismissible fade show text-danger" role="alert">
                                    <b>Error !</b> Receive quantity can't be equal to zero !.
                                    <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                                    </div>
                                `);
                                $(this).closest('tr').find('.rcv_qty').val(issue_qty);
                            }
                            if (rcv_qty <= issue_qty && rcv_qty != 0) {
                                // Build the URL, replacing mrf_detail_id and adding qty as a query parameter
                                let url =
                                    "{{ route('material_requisition.issue.ack', ':mrf_detail_id') }}";
                                url = url.replace(':mrf_detail_id', mrf_detail_id_ack);
                                url += '?qty=' + encodeURIComponent(
                                rcv_qty); // Append qty to the URL
                        ...
                        $('#productTable tbody').on('click', '.reject', function(e) {
                            ...
                            let url =
                                "{{ route('material_requisition.issue.reject', ':mrf_detail_id') }}";
        ...
        $('#productTable tbody').on('click', '.add-remarks3', function() {
            ...
            $('#remarksText3').prop('disabled', false); // Use prop with false
        ...
        $('#saveRemarks3').on('click', function() {
            ...
            selectedRow.node().querySelector('.rcv_remarks').value = remarks;
